...
export default function QSOList() {
  ...
  const normalizeQSOs = useCallback((data: QSO[]): NormalizedQSO[] => {
    if (!user?.call_sign) return [];

    return data.map(qso => {
      const isRecipient = qso.recipient === user.call_sign;

      return {
        id: qso.id,
        station1: isRecipient ? user.call_sign : qso.initiator_callsign,
        station2: isRecipient ? qso.initiator_callsign : qso.recipient,
        frequency: qso.frequency,
        mode: qso.mode,
        datetime: qso.datetime,
        location1: isRecipient ? qso.recipient_location : qso.initiator_location,
        location2: isRecipient ? qso.initiator_location : qso.recipient_location,
  ...
  }, [token, user?.call_sign, normalizeQSOs]);
